...
layout(location = 11) in uint64_t iAlbedo;
layout(location = 12) in uint64_t iNormal;
...
flat out uint64_t fAlbedo;
flat out uint64_t fNormal;
...
in vec3 vBitangent;
in vec3 vWorldPos;
flat in uint64_t fAlbedo;
flat in uint64_t fNormalMap;
...
{
    sampler2D albedoSampler = sampler2D(fAlbedo);
    vec4 albedoSample = texture(albedoSampler, vUV) * vColor;
    vec3 albedo = albedoSample.rgb;

    // normal map
    vec3 N = normalize(vNormal);
    if (fNormalMap != uint64_t(0))
    {
        sampler2D normalSampler = sampler2D(fNormalMap);
